...
const DetailScreen = ({navigation}: {navigation: any}) => {
  ...
  useEffect(() => {
    ...
    if (category == 'bp') {
      if (quesid != null && language?.article.articledata?.bp.questions[quesid] != undefined) {
        ...
        setbackground(language?.article.articledata?.bp.questions[quesid].background);
        setsubdescription(
    ...
    if (category == 'heart') {
      if (quesid != null && language?.article.articledata?.heart.questions[quesid] != undefined) {
        settitle(language?.article.articledata?.heart.questions[quesid].title);
        setsubtitle(language?.article.articledata?.heart.questions[quesid].contentTitle);
        setbackground(language?.article.articledata?.heart.questions[quesid].background);
        setsubdescription(language?.article.articledata?.heart.questions[quesid].contentDescription);
      }
    }
    if (category == 'bs') {
      if (quesid != null && language?.article.articledata?.bs.questions[quesid] != undefined) {
        settitle(language?.article.articledata?.bs.questions[quesid].title);
        setsubtitle(language?.article.articledata?.bs.questions[quesid].contentTitle);
        setbackground(language?.article.articledata?.bs.questions[quesid].background);
        setsubdescription(language?.article.articledata?.bs.questions[quesid].contentDescription);
      }
    }
  ...
  const customLoop = () => {
    let jsx = language?.article.articledata?.bp.questions.map((item:any, index:any) => {
      return (
        <TouchableOpacity
          activeOpacity={0.9}
    ...
    <View style={{flex: 1, backgroundColor: '#fff', padding: 0}}>
      ...
      <View style={styles.mainContainer}>
        <ScrollView
          ...
          {customLoop()}
          <TouchableOpacity
            onPress={() => navigation.navigate('HomeScreen', {tab: 'health'})}
            style={{alignSelf: 'center', marginVertical: 20, width: btnWidth, height: 176 * btnRatio, backgroundColor: '#5F45FE', alignItems: 'center',justifyContent: 'center', borderRadius: 10}}>
